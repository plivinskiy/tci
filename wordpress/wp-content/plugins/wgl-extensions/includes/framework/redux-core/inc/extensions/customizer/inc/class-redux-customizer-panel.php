	...
	protected function content_template() {
		...
		<li
			...
			<button class="customize-panel-back" tabindex="-1">
				<span class="screen-reader-text"><?php esc_attr_e( 'Back', 'wgl-extensions' ); ?></span>
			</button>
			<div class="accordion-section-title">
				<span class="preview-notice">
					<?php /* translators: %s is the site/panel title in the Customizer */ ?>
					<?php printf( esc_html__( 'You are customizing', 'wgl-extensions' ) . ' %s', '<strong class="panel-title">{{ data.title }}</strong>' ); ?>
				</span>
				<# if ( data.description ) { #>
				<button
					...
					tabindex="0"
					aria-expanded="false">
					<span class="screen-reader-text"><?php esc_attr_e( 'Help', 'wgl-extensions' ); ?></span>
				</button>
				<# } #>
			</div>
			<# if ( data.description ) { #>
		...
		<li id="accordion-panel-{{ data.id }}"
			...
			<h3 class="accordion-section-title">
				<?php if ( version_compare( $this->wp_ver, '6.7', '<' ) ) { ?>
					...
					<span class="accordion-trigger screen-reader-text"><?php echo esc_html__( 'Press return or enter to open this panel', 'wgl-extensions' ); ?></span>
				<?php } else { ?>
