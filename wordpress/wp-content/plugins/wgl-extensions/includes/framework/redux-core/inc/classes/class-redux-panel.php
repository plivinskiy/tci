		...
		public function get_template( string $file ) {
			...
			if ( file_exists( $path ) ) {
				if ( is_readable( $path ) ) {
					...
					echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux Panel Template %1$s cannot be read. Please check the permissions for this file.', 'wgl-extensions' ), '<code>' . esc_html( $path ) . '</code>' ) . '</p></div>';
				}
			} else {
				if ( file_exists( Redux_Core::$dir . 'templates/panel/' . $file ) ) {
					...
					// translators: %1$s: template path.
					echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux Panel Template %1$s does not exist. Please reinstall Redux to replace this file.', 'wgl-extensions' ), '<code>' . esc_html( $path ) . '</code>' ) . '</p></div>';
				}
			}
		}
				...
				if ( $developer_theme_file ) {
					...
					if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) /* && isset( $this->parent->args['dev_mode'] ) && ! empty( $this->parent->args['dev_mode'] ) */ ) {
						...
						<div id="message" class="error redux-message" style="display:block!important">
							<p>
								<strong><?php esc_html_e( 'Your panel has bundled copies of Redux Framework template files that are outdated!', 'wgl-extensions' ); ?></strong>&nbsp;&nbsp;<?php esc_html_e( 'Please ask the author of this theme to update them as functionality issues could arise.', 'wgl-extensions' ); ?>
							</p>
						</div>
						<?php
