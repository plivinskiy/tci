		...
		private function install_extendify(): bool {
			...
			if ( empty( $download_link ) ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Error: Install URL for Extendify could not be located.', 'wgl-extensions' ),
					)
				);
			...
			if ( true !== $install ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Install process for Extendify failed.', 'wgl-extensions' ),
					)
				);
			...
			if ( is_wp_error( $activate ) ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Extendify activation failed.', 'wgl-extensions' ),
					)
				);
			}
			...
			echo wp_json_encode(
				array(
					'type' => 'success',
					'msg'  => esc_html__( 'Extendify installed and activated.', 'wgl-extensions' ),
				)
			);
					...
					<div class="redux-banner-content-container">
						...
						<div class="redux-banner-slide redux-banner-slide-one redux-slide-is-active">
							...
							<div class="redux-banner-slide-text">
								<h2><?php esc_html_e( 'Action needed to continue using the template library.', 'wgl-extensions' ); ?></h2>
								<p>
									<?php
									esc_html_e(
								...
								<div class="redux-banner-button-container">
									...
									<a href="<?php echo esc_url( $urls['register'] ); ?>" data-url="<?php echo admin_url( 'admin-ajax.php' ); ?>" data-activate="main_banner"
									   class="button button-primary button-large redux-alt-connect-button redux-connection-banner-action">
										<?php esc_html_e( 'Install and Activate Extendify', 'wgl-extensions' ); ?>
									</a>
								</div>
