			...
			if ( isset( $_POST ) && isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_POST['nonce'] ) ), 'redux_submit_support' ) ) {
				...
				if ( is_wp_error( $nonce ) || empty( $nonce['body'] ) ) {
					echo wp_json_encode(
						array(
							'status' => 'error',
							'data'   => esc_html__( 'Security token', 'wgl-extensions' ) . ' ' . wp_remote_retrieve_response_code( $nonce ) . ': ' . wp_remote_retrieve_response_message( $nonce ),
						)
					);
				...
				if ( ! is_wp_error( $report ) && 200 === wp_remote_retrieve_response_code( $report ) && ! empty( $report['body'] ) ) {
					...
					$status = 'error';
					$data   = esc_html__( 'Data transmit', 'wgl-extensions' ) . ' ' . wp_remote_retrieve_response_code( $report ) . ': ' . wp_remote_retrieve_response_message( $report );
				}
