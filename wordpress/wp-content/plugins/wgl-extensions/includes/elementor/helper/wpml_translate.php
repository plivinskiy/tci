		...
		public static function get_translate($self, $widgets ) {
						...
						if('repeater' === $control['type']){
							...
							foreach( $control['fields'] as $key => $value ){
								...
								|| 'url' === $value['type']
									...
									$wpml_settings['fields'][] = [
										...
										'label' => $value['label'] ?? esc_html__('WGL Module', 'wgl-extensions'),
							...
							if('url' === $control['type']){
								$controls_data[$control['name']] = [
									'field'       => 'url',
									'type'        => $control['label'] ?? esc_html__('WGL Module', 'wgl-extensions'),
			...
			if(!empty($controls_data) || !empty($wpml_settings)){
				...
				if(!empty($wpml_settings)){
					$widgets[$self->get_name() ]['integration-class'] = new \WGL_Extensions\Includes\WGL_Elementor_Translate($wpml_settings);
				}
			}
			...
			if(!empty($this->wgl_fields['fields'])){
				foreach( $this->wgl_fields['fields'] as $key => $value ){
					...
					if($field === $name){
						...
						return esc_html__( $value['module_name'] . ': link URL', 'wgl-extensions' );
					}
				}
			}
