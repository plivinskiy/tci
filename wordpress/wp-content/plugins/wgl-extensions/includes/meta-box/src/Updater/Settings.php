...
class Settings {
	...
	public function add_settings_page() {
		...
		$title      = $this->option->is_network_activated() ? esc_html__( 'Meta Box License', 'wgl-extensions' ) : esc_html__( 'License', 'wgl-extensions' );
	...
	public function render() {
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Meta Box License', 'wgl-extensions' ); ?></h1>
			<p><?php esc_html_e( 'Please enter your license key to enable automatic updates for Meta Box extensions.', 'wgl-extensions' ); ?></p>
			<p>
				<?php
				printf(
					// Translators: %1$s - URL to the My Account page, %2$s - URL to the pricing page.
					wp_kses_post( __( 'To get the license key, visit the <a href="%1$s" target="_blank">My Account</a> page on metabox.io website. If you have not purchased any extension yet, please <a href="%2$s" target="_blank">get a new license here</a>.', 'wgl-extensions' ) ),
					'https://elu.to/mua',
					'https://elu.to/mup'
				);
				?>
			...
			<form action="" method="post">
				...
				<table class="form-table">
					<tr>
						<th scope="row"><?php esc_html_e( 'License Key', 'wgl-extensions' ); ?></th>
						<td>
							<?php
							$messages = [
								'invalid' => __( 'Your license key is <b style="color: #d63638">invalid</b>.', 'wgl-extensions' ),
								'error'   => __( 'Your license key is <b style="color: #d63638">invalid</b>.', 'wgl-extensions' ),
								'expired' => __( 'Your license key is <b style="color: #d63638">expired</b>.', 'wgl-extensions' ),
								'active'  => __( 'Your license key is <b style="color: #00a32a">active</b>.', 'wgl-extensions' ),
							];
				...
				<?php submit_button( __( 'Save Changes', 'wgl-extensions' ) ); ?>
			</form>
		</div>
		<?php
	}
	...
	public function save() {
		...
		if ( empty( $response ) ) {
			add_settings_error( '', 'mb-error', __( 'Something wrong with the connection to metabox.io. Please try again later.', 'wgl-extensions' ) );
		} elseif ( 'active' === $status ) {
			add_settings_error( '', 'mb-success', __( 'Your license is activated.', 'wgl-extensions' ), 'updated' );
		} elseif ( 'expired' === $status ) {
			// Translators: %s - URL to the My Account page.
			$message = __( 'License expired. Please renew on the <a href="%s" target="_blank">My Account</a> page on metabox.io website.', 'wgl-extensions' );
			$message = wp_kses_post( sprintf( $message, 'https://elu.to/mua' ) );

			add_settings_error( '', 'mb-expired', $message );
		} else {
			// Translators: %1$s - URL to the My Account page, %2$s - URL to the pricing page.
			$message = __( 'Invalid license. Please <a href="%1$s" target="_blank">check again</a> or <a href="%2$s" target="_blank">get a new license here</a>.', 'wgl-extensions' );
