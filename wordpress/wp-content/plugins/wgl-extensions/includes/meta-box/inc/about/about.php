...
class RWMB_About {
	...
	public function plugin_links( array $links ): array {
		$links[] = '<a href="' . esc_url( $this->get_menu_link() ) . '">' . esc_html__( 'About', 'wgl-extensions' ) . '</a>';
		if ( ! $this->is_pro ) {
			$links[] = '<a href="https://elu.to/mpp" style="color: #39b54a; font-weight: bold">' . esc_html__( 'Go Pro', 'wgl-extensions' ) . '</a>';
		}
		return $links;
	}
	...
	public function add_submenu(): void {
		...
		$about       = add_submenu_page(
			$parent_menu,
			__( 'Welcome to Meta Box', 'wgl-extensions' ),
			__( 'Dashboard', 'wgl-extensions' ),
			'activate_plugins',
			'meta-box',
			[ $this, 'render' ]
		);
