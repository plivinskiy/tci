...
class RWMB_File_Field extends RWMB_Field {
	public static function admin_enqueue_scripts() {
		...
		RWMB_Helpers_Field::localize_script_once( 'rwmb-file', 'rwmbFile', [
			...
			'maxFileUploadsSingle' => __( 'You may only upload maximum %d file', 'wgl-extensions' ),
			// Translators: %d is the number of files in plural form.
			'maxFileUploadsPlural' => __( 'You may only upload maximum %d files', 'wgl-extensions' ),
		] );
	}
	...
	public static function ajax_delete_file() {
		...
		if ( ! self::in_array_r( $attachment, $field_value ) ) {
			wp_send_json_error( __( 'Error: Invalid file', 'wgl-extensions' ) );
		}
		// Delete the file.
		...
		wp_send_json_error( __( 'Error: Cannot delete file', 'wgl-extensions' ) );
	}
	...
	public static function html( $meta, $field ) {
		...
		$i18n_more = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'wgl-extensions' ), $field );
	...
	protected static function file_html( $file, $index, $field ) {
		$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'wgl-extensions' ) );
		$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'wgl-extensions' ) );
