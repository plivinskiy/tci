...
class RWMB_Taxonomy_Field extends RWMB_Object_Choice_Field {
	...
	public static function normalize( $field ) {
		 ...
		 */
		$placeholder   = __( 'Select a term', 'wgl-extensions' );
		$taxonomy_name = self::get_taxonomy_singular_name( $field );
		if ( $taxonomy_name ) {
			// Translators: %s is the taxonomy singular label.
			$placeholder = sprintf( __( 'Select a %s', 'wgl-extensions' ), strtolower( $taxonomy_name ) );
		}
		$field = wp_parse_args( $field, [
	...
	public static function query( $meta, array $field ): array {
		...
		foreach ( $terms as $term ) {
			$label = $term->name ? $term->name : __( '(No title)', 'wgl-extensions' );
