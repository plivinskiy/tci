...
class RWMB_Input_List_Field extends RWMB_Choice_Field {
	...
	public static function get_select_all_html( $field ) {
		if ( $field['multiple'] && $field['select_all_none'] ) {
			return sprintf( '<p class="rwmb-toggle-all-wrapper"><button class="rwmb-input-list-select-all-none button" data-name="%s">%s</button></p>', $field['id'], __( 'Toggle All', 'wgl-extensions' ) );
		}
		return '';
	}
}
