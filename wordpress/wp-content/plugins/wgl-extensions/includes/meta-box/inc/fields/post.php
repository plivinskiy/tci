...
class RWMB_Post_Field extends RWMB_Object_Choice_Field {
	...
	public static function normalize( $field ) {
		 ...
		 */
		$placeholder = __( 'Select a post', 'wgl-extensions' );
		if ( 1 === count( $field['post_type'] ) ) {
			$post_type        = reset( $field['post_type'] );
			$post_type_object = get_post_type_object( $post_type );
			if ( ! empty( $post_type_object ) ) {
				// Translators: %s is the post singular label.
				$placeholder = sprintf( __( 'Select a %s', 'wgl-extensions' ), strtolower( $post_type_object->labels->singular_name ) );
			}
		}
		$field = wp_parse_args( $field, [
	...
	public static function query( $meta, array $field ): array {
		...
		foreach ( $query->posts as $post ) {
			...
			$label                = $post->post_title ? $post->post_title : __( '(No title)', 'wgl-extensions' );
