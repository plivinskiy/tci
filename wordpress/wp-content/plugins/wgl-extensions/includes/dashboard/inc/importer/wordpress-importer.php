...
if (class_exists('WP_Importer')) {
    class WGL_WP_Import extends WP_Importer
    {
        ...
        var $rev_slider = '';
        var $wgl_elementor_library = array();
        ...
        function import($file, $last_item = false)
        {
            ...
            $this->import_wgl_elementor_library();
            $this->import_end();
        }
        ...
        function import_wgl_elementor_library()
        {
            if (
                !empty($this->wgl_elementor_library) &&
                class_exists('\WGL_Elementor_Templates\Custom_Library\Core\Library_Manager')
            ) {
                foreach ($this->wgl_elementor_library as $post_id) {
                    \WGL_Elementor_Templates\Custom_Library\Core\Library_Manager::get_instance()->add_template_to_library($post_id);
                }
            }
        }
...
<?php
            ...
            foreach ($this->posts as $post) {
                ...
                if (!empty($post['postmeta'])) {
                    foreach ($post['postmeta'] as $meta) {
                        ...
                        if('wgl_elementor_custom_library_sync_to_library' === $meta['key']){
                            $data_library = $meta['value'];
                            if(!empty($data_library)){
                                $this->wgl_elementor_library[] = $post_id;
                            }
                        }
                    }
                }
        ...
        function fetch_remote_file($url, $post)
        {
            ...
            if(isset($GLOBALS['skip_image_demo_content'])){
                ...
                if (strpos($url, 'wgl-demo.net') !== false) {
                    $url = str_replace('https:\/\/' . $theme_name . '.wgl-demo.net', 'https://' . $theme_name . '.wgl-dsites.net', $url);
                    $url = str_replace('https://' . $theme_name . '.wgl-demo.net', 'https://' . $theme_name . '.wgl-dsites.net', $url);
                }else{
                    $url = str_replace('https:\/\/' . $theme_name . '.wgl-dsites.net', 'https:\/\/webgeniuslab.net\/images\/' . $theme_name , $url);
                    $url = str_replace('https://' . $theme_name . '.wgl-dsites.net', 'https://webgeniuslab.net/images/' . $theme_name, $url);                    
                }

                //* Add SubFolders Compatibility
                if (strpos($url, 'wgl-demo.net') !== false) {
                    $url = str_replace('https:\/\/wgl-demo.net\/' . $theme_name, 'https:\/\/wgl-dsites.net\/' . $theme_name, $url);
                    $url = str_replace('https://wgl-demo.net/' . $theme_name, 'https://wgl-dsites.net/' . $theme_name, $url);
                }else{
                    //* Add new domain SubFolders Compatibility
                    $url = str_replace('https:\/\/wgl-dsites.net\/' . $theme_name, 'https:\/\/webgeniuslab.net\/images\/' . $theme_name , $url);
                    $url = str_replace('https://wgl-dsites.net/' . $theme_name, 'https://webgeniuslab.net/images/' . $theme_name, $url);                    
                }
            }
            ...
            /*
            if (404 === $remote_response_code) {
                if(isset($_REQUEST['error_log']) && isset($GLOBALS['skip_image_demo_content'])){
                    $error_message = sanitize_text_field( wp_unslash( $_REQUEST['error_log'] ) );
                    echo '<p class="notice notice-error">' . esc_html( $error_message ) . '</p>';
                    die();
                }
            }
            */
