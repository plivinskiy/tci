        ...
        function loadContent(parent, data, reimport) {
            jQuery.post(ajaxurl, data, function(response) {
                if (response.length > 0 && response.match(/Have fun!/gi)) {
                    ...
                    parent.closest('.wbc_importer').find('#info-opt-info-error').find('.error_message').html(response);
                    let error = parent.closest('.wbc_importer').find('#info-opt-info-error').find('.error_message').find('.notice-error');
                    parent.closest('.wbc_importer').find('#info-opt-info-error').find('.error_message').html(error);
                    parent.find('.wbc_image').css('opacity', '1');
                    parent.closest('.wbc_importer').find('.importer_status').css('opacity', '0');
                    parent.closest('.wbc_importer').find('#info-opt-info-error').show('slow');
                    parent.find('.spinner').css('display', 'none');
                    parent.find('.importer-button').show();

                    if(parent.closest('.wbc_importer').find('#info-opt-info-error').find('#without_image').is(":checked")){
                        data.without_image = true;
                        parent.closest('.wbc_importer').find('.importer_status').css('opacity', '1');
                        parent.find('.wbc_image').css('opacity', '0.5');
                        parent.closest('.wbc_importer').find('#info-opt-info-error').hide();
                        loadContent(parent, data);
                    }
