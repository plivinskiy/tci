...
{
    return apply_filters('theme_helper_function/user_social',
        [
            'twitter' => esc_html__('Twitter', 'wgl-extensions'),
            'facebook-f' => esc_html__('Facebook', 'wgl-extensions'),
            'instagram' => esc_html__('Instagram', 'wgl-extensions'),
            'linkedin-in' => esc_html__('Linkedin', 'wgl-extensions'),
            'telegram-plane' => esc_html__('Telegram', 'wgl-extensions')
        ]
    );
}
...
function wgl_extra_user_profile_fields($user)
{
    ...
    echo '<h3>', esc_html__('Social media accounts', 'wgl-extensions'), '</h3>';

    echo '<table class="form-table">';
        foreach (wgl_user_social_medias_arr() as $social => $value) {
            ?>
            <tr>
                <th><label for="<?php echo esc_attr($social); ?>" style="text-transform: capitalize;"><?php esc_html_e( $value, 'wgl-extensions' ); ?></label></th>
                <td>
                    ...
                    <span class="description"><?php esc_html_e('Your '.$value.' url.', 'wgl-extensions'); ?></span>
                </td>
            </tr>
            <?php
        }
...
function wgl_wp_head_custom_code()
{
    ...
    if (class_exists('WGL_Framework')) {
        $header_custom_code = WGL_Framework::get_option('header_custom_js');
    }
...
{
    if (class_exists('WGL_Framework')) {
        $custom_js = WGL_Framework::get_option('custom_js');
    }
