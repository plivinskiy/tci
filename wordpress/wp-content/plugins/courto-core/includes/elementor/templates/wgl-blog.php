<?php
/**
 * This template can be overridden by copying it to `yourtheme[-child]/wgl-extensions/elementor/templates/wgl-blog.php`.
 */
...
class WGL_Blog
{
    ...
    public function render($_this, $attributes)
    {
        ...
        if (isset($this->attributes['cursor_tooltip']) && (bool)$this->attributes['cursor_tooltip']) {
        ...
        $cursor_data .= isset($this->attributes['cursor_tooltip']) && (bool)$this->attributes['cursor_tooltip'] ? ' data-cursor-event=".wgl-cursor-event"' : '';

        echo '<div class="wgl_cpt_section">';

        echo '<div class="blog-posts' . ( isset($this->attributes['cursor_tooltip']) && (bool)$this->attributes['cursor_tooltip'] ? ' wgl-cursor-text' : '' ) . '"' . $cursor_data . '>';

        $this->render_header_section();

        echo '<div class="container-grid', $this->get_row_classes(), '">',
            $this->get_posts_html(),
        '</div>';
    ...
    public function support_archive_tax()
    {
        ...
        if(!empty($term_id) && is_archive() && ! empty( $post->post_type ) && 'post' === $post->post_type){
            if(is_category()){
                $taxonomies[] = $tax_obj->slug;
                $this->attributes['categories'] = $taxonomies;
            }else if (is_tag()){
                $taxonomies[] = $tax_obj->slug;
                $this->attributes['tags'] = $taxonomies;
            }else{
    ...
    protected function render_header_section()
    {
        ...
        if ($module_title) {
            echo '<h4 class="courto_module_title blog_title">',
