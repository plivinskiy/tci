    ...
    class WGL_Theme_Verify
    {
        ...
        */
        ...
        public function notices_validation(){
            ...
            ?>
            <div class="notice notice-error is-dismissible wgl-notice">
                <p>
                    <?php echo wp_get_theme()->get('Name');
                        echo sprintf( esc_html__( ' Theme is not activated! Please activate your copy of the %s Theme.', 'courto'), wp_get_theme()->get('Name') );
                        ?>
                </p>
                <p>
                    <strong class="activate_error"><?php esc_html_e( 'Please activate the theme!', 'courto' ); ?></strong> -
                    <a href="<?php echo esc_url(( $page_purchase )); ?>">
                        <?php esc_html_e( 'Activate Now','courto' ); ?>
                    </a>
                </p>
            </div>
            ...
            <div class="notice notice-warning is-dismissible">
                <p class="support-until_text">
                    <?php
                        ...
                        if($now > $until_date){
                            echo sprintf( wp_kses( __( '<strong>Your support period expired. <a href="%s">You can prolong support</a> services as you need.</strong>', 'courto' ), $allowed_html), esc_url($item_url));
                        }elseif($now < $until_date && $interval->days < 30){
                            echo sprintf( wp_kses( __( '<strong>Your support period will be expired after %d days. <a href="%s">You can prolong support</a> services as you need.</strong>', 'courto' ), $allowed_html), $interval->days, esc_url($item_url));
                        }
                ...
                <p>
                    ...
                    ?>
                    <a href="<?php echo esc_url($remind_url); ?>" class="button button-primary"><?php echo esc_html__( 'Remind me later', 'courto' ); ?></a>&nbsp;
                    <a href="<?php echo esc_url($delete_url); ?>" class="button button-primary"><?php echo esc_html__( 'Hide and disable further reminders', 'courto' ); ?></a>
                </p>
            </div>
        ...
        public function purchase_activation(){
            ...
            if ( ! isset( $_POST['email'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-activation' ) ) {
                $output['error'] = 1;
                $output['message'] = esc_html__( 'Please enter a valid field', 'courto' );
                ...
                if( ! is_email( $email ) ){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Please enter a valid email address.', 'courto' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if(empty($purchase)){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Purchase code is empty ', 'courto' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if( $return !== false ){
                    ...
                    if(isset($result['success']) && !empty($result['success'])){
                        ...
                        $output['message'] =  esc_html__( 'Thank you, your license has been validated', 'courto' );
                        ...
                        $output['message'] .= '<span class="register_purchase--title">'.esc_html__( 'This purchase code has been registered.', 'courto' ).'</span><br/>';
                        $output['message'] .= esc_html__( 'Please go to your previous working environment and deactivate the purchase code to use it again (WP dashboard -> WebGeniusLab -> Activate Theme -> click on the button "Deactivate" ).', 'courto' ).'<br/><br />';
        ...
        public function reset_purchase(){
            ...
            if ( ! isset( $_POST['email'] ) || ! isset( $_POST['accout'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-reset-activation' ) ) {
                ...
                $email      = sanitize_email($_POST['email']);
                $purchase   = sanitize_text_field($_POST['purchase_code']);
                $accout     = sanitize_text_field($_POST['accout']);
                ...
                if(empty($accout)){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Your Envato Account is empty ', 'courto' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if (false === $response) {
                    ...
                    if( $return !== false ){
                        ...
                        if(isset($result['success']) && !empty($result['success'])){
                            $output['success'] = 1;
                            $output['error'] = 0;
                            $output['message'] =  esc_html__( 'Your Request Has been Sent', 'courto' );
                    ...
                    $output['success'] = '';
                    $output['message'] = esc_html__( 'The request has been sent before. You will get an email when it is reviewed.', 'courto' );
                    $output['error'] = 1;
                    echo json_encode( $output );
                }
            }
        ...
        public function js_activate_theme(){
			...
			if($alternative_activation){
				$output['purchase'] = sanitize_text_field($_POST['purchase_item']);
				$output['success'] = 1;
				$output['email'] = sanitize_email($_POST['user_email']);
				$output['error'] = '';
				...
				update_option( WGL_Theme_Verify::get_instance()->item_id, sanitize_text_field($_POST['purchase_item']) );
			}
		}
