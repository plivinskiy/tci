...
if (!class_exists('Courto_Single_Post')) {
    /**
     * @package courto\core\class
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     */
    class Courto_Single_Post
    {
	    /**
	     * @var Courto_Single_Post
	     */
            ...
            if (class_exists('RWMB_Loader')) {
                switch ($this->post_format) {
                    ...
                    case 'video':
                        ...
                        if($this->media && (bool) strpos($this->media, 'rwmb-oembed-not-available', 11)){
                            $this->media = false;
                        }
                        break;
                    case 'quote':
                        $this->media = $this->featured_quote();
                        if (is_single() && isset($wgl_related_posts) && empty($wgl_related_posts)) {
                            $this->meta_info_render = true;
                        } else {
                            $this->meta_info_render = false;
                        }
        ...
        public function render_featured(...$args)
        {
            ...
            echo '<div class="blog-post_media">';
                echo '<div class="blog-post_media_part', esc_attr($this->media_part_class), '">',
                    $this->media,
                '</div>';
                if (!$args['hide_all_meta'] && $this->meta_info_render && !empty($args['meta_cats'])) {
                        ...
                        $output .= '<div class="videobox_content">';
                            ...
                            if ($video_link) {
                                ...
                                $output .= '<div ' . $this->get_render_attribute_string( 'video-lightbox' ) . '>';
                                    $output .= '<svg class="videobox_icon" viewBox="-20 -20 272 272"><path d="M203,99L49,2.3c-4.5-2.7-10.2-2.2-14.5-2.2 c-17.1,0-17,13-17,16.6v199c0,2.8-0.07,16.6,17,16.6c4.3,0,10,0.4,14.5-2.2 l154-97c12.7-7.5,10.5-16.5,10.5-16.5S216,107,204,100z"/></svg>';
                                $output .= '</div>';
                                $output .= '</div>';
                            }
        ...
        public function featured_gallery($image_size)
        {
            ...
            echo '<div class="wgl-carousel_wrapper">';
                ...
                echo '<div class="wgl-navigation_wrapper">';
                    echo '<button class="elementor-swiper-button elementor-swiper-button-prev" data-carousel="' . esc_attr($carousel_item) . '"><i class="wgl-svg-icon">'.wgl_dynamic_styles()->wgl_theme_svg()['arrow-right'].'</i></button>';
                    echo '<button class="elementor-swiper-button elementor-swiper-button-next" data-carousel="' . esc_attr($carousel_item) . '"><i class="wgl-svg-icon">'.wgl_dynamic_styles()->wgl_theme_svg()['arrow-right'].'</i></button>';
                echo '</div>';
            echo '</div>';
        ...
        public function render_post_meta($args = false, $wrapper = true)
        {
            ...
            if ($this->show_meta_author
                ...
                echo (true == $wrapper) ? '<div class="meta-data">' : '';
                    ...
                    if ($this->show_meta_author) {
                        $avatar = get_avatar( get_the_author_meta( 'ID' ) , 92 );
                        echo '<span class="post_author"><span>',
                            esc_html__('by ', 'courto'),
                    ...
                    if ($this->show_meta_comments) {
                        ...
                        echo '<span class="comments_post">',
                            '<a href="', esc_url(get_comments_link()), '" title="', esc_attr__('Leave a reply', 'courto'), '">',
                                esc_html($comments_num),
                                ' ',
                                '<span class="comments_text">', esc_html(_n('Comment', 'Comments', $comments_num, 'courto')), '</span>',
                            '</a>',
                        '</span>';
                    }
        ...
        public function render_author_info()
        {
            ...
            if (function_exists('wgl_user_social_medias_arr')) {
                foreach (wgl_user_social_medias_arr() as $social => $value) if (get_the_author_meta($social)) {
                    switch ($social) {
                        case 'twitter':
                            $social_html = esc_html__('X', 'courto');
                            break;
                        case 'facebook-f':
                            $social_html = esc_html__('Facebook', 'courto');
                            break;
                        case 'instagram':
                            $social_html = esc_html__('Instagram', 'courto');
                            break;
                        case 'linkedin-in':
                            $social_html = esc_html__('Linkedin', 'courto');
                            break;
                        case 'telegram-plane':
                            $social_html = esc_html__('Telegram', 'courto');
                            break;
                        default:
                            $social_html = '';
                            break;
            ...
            if ($name_html && $description) :
                echo '<div class="author-info_wrapper">',
                    $avatar_html,
